...
;;;     specialized on CLIMB:SELECTION-OBJECT. Arugment TYPE should be ignored.
;;;
...
;;;   4. Look for a matching translator in the global table.
    ...
    (alexandria:when-let ((object (stored-object port selection)))
      (let ((selected-type (selection-object-type object))
        ...
        (flet ((get-table (object)
          ...
          (let ((table-1 (get-table object))
                (table-2 (get-table request))
                (table-3 (find-command-table 'global-command-table))
            ...
            (some (lambda (tr)
                  ...
                  (remove-duplicates
                   (append
                    (find-presentation-translators selected-type requested-type table-1)
                    (find-presentation-translators selected-type requested-type table-2)
                    (find-presentation-translators selected-type requested-type table-3))))))))))

#+test
(macrolet ((is (form) `(assert ,form)))
  (let ((port (make-instance 'standard-port))
