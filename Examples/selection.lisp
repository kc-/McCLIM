    ...
    (macrolet ((thunk (type)
                 `(formatting-cell (pane)
                    (multiple-value-bind (object ptype)
                        (request-selection pane :clipboard ,type)
                      (if object
                          (present object (or ptype (presentation-type-of object)))
                          (format pane "(no translation)")))))
               (thunks (&rest types)
                 `(formatting-row (pane)
...
(define-selection-demo-command (com-paste :menu t) ()
  (setf (pasted-element *application-frame*)
        (request-selection *standard-output* :clipboard t)))
